...
class HW2ClassificationTransfer(Learning):
    def __init__(self, params: ParamsHW2Classification, model):
        super().__init__(params, model, None, nn.CrossEntropyLoss)
        self.optimizer = torch.optim.Adam(self.model.net._fc.parameters(), lr=self.params.lr)
        print(str(self))
...
def main():
    ...
    learner = HW2ClassificationTransfer(params, model)
    if args.epoch >= 0:
        if args.load == '':
